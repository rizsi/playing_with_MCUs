...
static bool rxRead(uint32_t * outCounter, uint8_t *outError, uint32_t * outZero)
{
	if(rxCtr>=9*8)
	{
		*outCounter=(((uint32_t)rxBytes[0])<<0)|
				(((uint32_t)rxBytes[1])<<8)|
			(((uint32_t)rxBytes[2])<<16)|
			(((uint32_t)rxBytes[3])<<24);
		*outError=rxBytes[4];
		*outZero=(((uint32_t)rxBytes[5])<<0)|
			(((uint32_t)rxBytes[6])<<8)|
			(((uint32_t)rxBytes[7])<<16)|
			(((uint32_t)rxBytes[8])<<24);
		return true;
	}else
	{
		*outCounter=0;
...
static void checkBreakpoint(AVR_simulator_t * ctx, uint32_t pc)
{
	...
	if(pc==comm_read)
	{
		if(rxTriggered)
		{
			...
			// printf("Read bit %d %d %d\n", byte, bit, rxval);
			if(rxval && byte<sizeof(rxBytes))
			{
				rxBytes[byte]|=1<<bit;
			}
			rxCtr++;
