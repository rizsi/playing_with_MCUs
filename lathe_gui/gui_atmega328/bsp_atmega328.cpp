...
static int8_t finishPinPadADC()
{
	...
	if((ADCSRA&_BV(ADIF)) != 0)
	{
		...
		ADCSRA|=_BV(ADIF);
		uint16_t value=ADC;
//		UART0_Send_uint32(value);
//		UART0_Send('\n');
		// Generated by Divisor.java
		if(value>=82&&value<181) pressedIndex=11; // '#' 0.400390625V-0.8837890625V
		if(value>=181&&value<220) pressedIndex=0; // '0' 0.8837890625V-1.07421875V
		if(value>=220&&value<264) pressedIndex=10; // '*' 1.07421875V-1.2890625V
		if(value>=264&&value<307) pressedIndex=9; // '9' 1.2890625V-1.4990234375V
		if(value>=307&&value<362) pressedIndex=8; // '8' 1.4990234375V-1.767578125V
		if(value>=362&&value<453) pressedIndex=7; // '7' 1.767578125V-2.2119140625V
		if(value>=453&&value<539) pressedIndex=6; // '6' 2.2119140625V-2.6318359375V
		if(value>=539&&value<600) pressedIndex=5; // '5' 2.6318359375V-2.9296875V
		if(value>=600&&value<676) pressedIndex=4; // '4' 2.9296875V-3.30078125V
		if(value>=676&&value<740) pressedIndex=3; // '3' 3.30078125V-3.61328125V
		if(value>=740&&value<786) pressedIndex=2; // '2' 3.61328125V-3.837890625V
		if(value>=786) pressedIndex=1; // '1' 3.837890625V--0.0048828125V
	}else
	{
		UART0_Send('X');
	}
