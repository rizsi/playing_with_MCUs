...
The envelope of the line is the same independent of the charging frequency (using the same 50% PWM duty in each case) but higher frequency means less noise on the signal.
...
Eventually using too small time periods degrades performance. See images half period is: 5us, 1us

== Effects of changing duty cycle

With shorted active cycles the output voltage becomes less (using the same output load)
Images: active vs inactive (us) 10 vs 0, 6 vs 4, 2 vs 8
